...
3. Hit **Generate Reels**. The passage is covered twice over by reels of randomized, variable length (10-30 consecutive ayahs each, never crossing a surah boundary) — so every ayah turns up in two different reels, each time with different neighbors — and the reels are then shuffled into non-sequential order (a reel covering ayah 15-30 might be followed by one covering ayah 110-125). Each reel cycles through a shuffled mix of your selected reciters and one of 16 scenery backdrops. The same logic applies to Surah, Page, and Range modes; reel count scales with passage length and never depends on how many reciters are selected.
