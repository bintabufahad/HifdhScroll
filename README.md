...
3. Hit **Generate Reels**. For X ayahs in the passage, exactly 2X reels are generated — each reel is a single ayah, and every ayah gets two reels (each time with a different reciter/scenery pairing) — then the whole list is shuffled into non-sequential order (a reel for ayah 30 might be followed by one for ayah 112, then one for ayah 58). The same logic applies to Surah, Page, and Range modes; reel count scales with passage length and never depends on how many reciters are selected.
