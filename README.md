...
- **Camera self-view** (`src/components/study/CameraSelfView.tsx`): opt-in (a button, never auto-requested); the `<video>` element is always mounted so the stream attaches reliably, and the stream is only ever rendered locally — nothing is uploaded or recorded.
- **Structured course playlist** (`src/components/study/CoursePlaylist.tsx`): a floating panel (top-right) to build an ordered course of YouTube lectures and PDF readings, reorder/check them off, and see progress. Clicking a lecture plays it in the teacher panel; a PDF opens in a new tab. Stored in localStorage.
