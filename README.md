...
- **Layout**: on wide screens it's a full-height two-column workspace — the lecture fills the left at a large size, and the camera plus the (prominent) to-do list fill a right column, with the compact focus timer at the bottom corner, so nothing needs scrolling or fullscreen. On mobile everything stacks and scrolls.
