...
- **Simulated class + lecture** (`src/components/study/LecturePanel.tsx`): the "teacher" panel shows a static placeholder until a YouTube link is added (typed in, or picked from the course playlist), then plays that lecture in the teacher's place via `youtube-nocookie.com`. Clearly labeled throughout as a simulation ("not a real class, no one else is on this call"). When a lecture is playing, the dashboard reflows so the class fills the space, the camera sits beneath it, and the timer + to-do go below that.
- **Camera self-view + local recording** (`src/components/study/CameraSelfView.tsx`): opt-in (a button, never auto-requested); the `<video>` element is always mounted so the stream attaches reliably. Optional recording uses the MediaRecorder API and stays entirely on the device — the clip is held in memory as a blob the user can replay or download; it is never uploaded or sent anywhere.
- **Faceless "Ustad" watcher** (`src/components/study/UstadWatcher.tsx`): a purely decorative faceless robed figure that periodically peeks in from the screen edge as a light "someone is watching, stay focused" nudge. It doesn't track anything.
- **Structured course playlist** (`src/components/study/CoursePlaylist.tsx`): a floating panel (top-right) to build an ordered course of YouTube lectures and PDF readings, reorder/check them off, and see progress. YouTube items show their cover thumbnail so it's clear which video is which. Clicking a lecture plays it in the teacher panel; a PDF opens in a new tab. Stored in localStorage.
- **Layout**: on wide screens the lecture sits on the left with the camera, timer, and to-do stacked in a right column so they stay in view without scrolling. The hub (`/`) and study dashboard use a modern dark theme (near-black with emerald/gold accents, glass panels — see `.bg-app-dark`/`.glass` in `src/app/globals.css`); the reel/waitlist/feedback pages still use the original parchment look.
- **Background audio** (`src/components/study/MusicPlayer.tsx`): a floating music button (top-right) to queue direct Qur'an-audio (mp3) links that play while studying. Stored in localStorage.
