...
export default function HomeHub() {
  ...
  return (
    <div dir={dir} className="bg-app-dark flex flex-1 flex-col items-center justify-center px-5 py-8 sm:py-12">
      ...
      {/* Corner actions: the emotional mushaf + gift an ayah. */}
      <div className="fixed right-3 top-3 z-50 flex items-center gap-2 sm:right-4 sm:top-4">
        <Link
          href="/feel"
          className="lift glass-strong flex h-10 items-center gap-1.5 rounded-full px-3 text-amber-100 shadow-md transition hover:text-white sm:h-11"
        >
          <span className="text-lg">🕊️</span>
          <span className="hidden text-xs font-medium sm:inline">{t("feelTitle")}</span>
        </Link>
        <Link
          href="/gift/new"
          className="lift glass-strong flex h-10 items-center gap-1.5 rounded-full px-3 text-emerald-200 shadow-md transition hover:text-white sm:h-11"
        >
